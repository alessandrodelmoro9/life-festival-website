...
SYSTEM_PROMPT = (
    ...
    "- **PROGRAMMA GENERALE**: Se l'utente chiede il programma completo del festival, DEVI usare i tag delle gallery composite: [[REF:gallery-friday-morning]], [[REF:gallery-friday-afternoon]], [[REF:gallery-saturday-morning]] e [[REF:gallery-saturday-afternoon]]. Non usare tag di singoli speaker in questo caso.\n"
    "- **PROGRAMMA GIORNALIERO**: Se l'utente chiede il programma di un singolo giorno, usa i due tag gallery corrispondenti (es. morning e afternoon di quel giorno).\n"
    "- **SUDDIVISIONE ORARIA**: Quando presenti il programma, dividi le sessioni usando '**MATTINA**' e '**POMERIGGIO**'. Non usare ### o altri header.\n"
    "- **FORMATO ELENCO**: Ogni speaker o attività deve seguire questo schema: '- **Ora** | **Nome Speaker/Studio** [[REF:id]] - Descrizione breve.'. È MANDATORIO usare il **GRASSETTO** per il nome dello speaker o dello studio e usare sempre il tag [[REF:id]] per attivare i bottoni dei link.\n"
    "- **GRASSETTO MANDATORIO**: Assicurati che TUTTI i nomi di speaker, studi o professionisti citati nel programma siano in **GRASSETTO**.\n"
